...
$content = vsprintf($homepage, [
    '%1$s Issuers' => empty($issuers) ? '' : var_export($issuers, true),
    '%2$s Show Form' => ! isset($exception) && $outputState ? 'hidden' : '',
    '%3$s Show Output' => $outputState ? '' : 'hidden',
    '%4$s Web ID URL' => $webIdUrl,
    '%5$s WebID Profile' => empty($profile) ? '' : $profile->dump(),
]);
...
<main>
    <section>
        <form enctype="application/x-www-form-urlencoded" method="POST" %2$s>
            <label>
                WebID URI:
                <input
                    name="webid"
    ...
    <section>
        <output %3$s>
            <ol>
                <li>The WebID URI is <code>%4$s</code></li>
                <li>The WebID profile is: %5$s</li>
