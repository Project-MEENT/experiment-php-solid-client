...
/**
 * Example of how to fetch a protected resource for a user authenticated with OpenID Connect.
 *
...
function findHttpResponse($trace, &$origin, &$dump)
{
    ...
    if (
        ...
        if (empty($message)) {
            ...
            $message = htmlentities($message); // @CHECKME: Or $message = strip_tags($message); (?)
        }
        $dump .= ''
...
if(isset($privateContainerUrl)) {
    ...
    try {
        ...
        error($e, "Could not fetch protected resource from $privateContainerUrl");
        exit;
    }
}
// =============================================================================
...
$content = vsprintf($homepage, [
    ...
    '%18$s Post-Redirect Hidden' => $isRedirect ? '' : 'hidden',
]);
        ...
        <output %2$s>
            <ol>
                ...
                <li %18$s data-webid="%16$s">
                    ...
                    <p>Use this WebID <a href="/oidc-offline-access/?webid=%16$s">in the Access a protected resource offline Example</a></p>
                </li>
                <li %18$s>Protected resource<pre><code>%17$s</code></pre></li>
                <li %18$s>
                    Use this issuer <a href="/oidc-offline-access/?issuer=%3$s">in the Access a protected resource offline Example</a>
                </li>
            </ol>
        </output>
    </section>
